...
class ImprovedCustomerSimulator:
    ...
    async def generate_response(self, history: List[Dict[str, str]], expected_question: str = None) -> str:
        ...
        intent = self._detect_livia_intent(last_livia_message, expected_question)
    ...
    def _detect_livia_intent(self, livia_message: str, expected: str = None) -> str:
        ...
        text = (expected or livia_message or "").lower()
        
        # Padrões de detecção melhorados
        if any(pattern in text for pattern in ["nome", "como se chama", "qual seu nome"]):
            return "asking_name"
        
        if any(pattern in text for pattern in ["e-mail", "email", "endereço eletrônico"]):
            return "asking_email"
        
        if any(pattern in text for pattern in ["seguro", "interesse", "produto", "tipo de seguro"]):
            return "asking_product"
        
        if any(pattern in text for pattern in ["horário", "dia", "quando", "ligar", "retorno"]):
            return "asking_schedule"
        
        if any(pattern in text for pattern in ["tchau", "obrigad", "encerrar"]):
            return "ending"
        
        # Detecta se Livia está repetindo pergunta
    ...
    async def _respond_to_name_request(self, attempt: int) -> str:
        ...
        # Aplica resistência se for persona cética
        if self.personality.config["resistance_prob"] > 0.3 and random.random() < 0.2:
            return random.choice([
        ...
        elif self.persona_type == "formal":
            return f"Meu nome é {self.name}."
        else:
            return random.choice([
                f"Sou {self.name}.",
    ...
    async def _respond_to_email_request(self, attempt: int) -> str:
        ...
        # Resistência
        if random.random() < self.personality.config["resistance_prob"] * 0.5:
            return random.choice([
